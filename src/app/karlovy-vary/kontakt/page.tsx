          ...
          <div className="section-container">
            ...
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
              ...
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
                <div className="card-maker p-8 space-y-6">
                  <h2 className="text-xl font-semibold text-ink">Spojte se s námi</h2>
                  <div className="space-y-4">
                    <a href={`mailto:${location.contact.email}`} className="flex items-center gap-3 text-ink-500 hover:text-primary-600 transition-colors">
                      <Mail className="w-5 h-5 text-primary-500" />
                      <span>{location.contact.email}</span>
                    </a>
                    <a href={`tel:${location.contact.phone.replace(/\s/g, '')}`} className="flex items-center gap-3 text-ink-500 hover:text-primary-600 transition-colors">
                      <Phone className="w-5 h-5 text-primary-500" />
                      <span>{location.contact.phone}</span>
                    </a>
                  </div>
                  <div className="pt-4 border-t border-ink/10">
                    <h3 className="font-medium text-ink mb-2">Organizátor</h3>
                    <p className="text-ink-500">{location.organizer.fullName}</p>
                  </div>
                </div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
                <div className="card-maker p-8 space-y-4">
                  <h2 className="text-xl font-semibold text-ink">Místo konání</h2>
                  <div>
                    <h3 className="font-medium text-ink">{venue.name}</h3>
                    <p className="text-sm text-ink-500">{venue.fullName}</p>
                  </div>
                  ...
                  <p className="text-sm text-ink-500">{venue.description}</p>
                  {venue.url && (
