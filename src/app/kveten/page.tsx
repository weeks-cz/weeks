...
function CampCard({ camp, index }: { camp: typeof CAMPS[number]; index: number }) {
  ...
  return (
    ...
    >
      <div className={`h-full bg-gradient-to-br ${camp.gradient} backdrop-blur rounded-md p-5 sm:p-8 border border-white/20 flex flex-col`}>
        {/* Header */}
        <div className="flex items-center gap-3 mb-4">
          <div className={`w-12 h-12 rounded-sm ${camp.iconBg} flex items-center justify-center border border-white/30`}>
            <Icon className="w-6 h-6 text-white" />
          </div>
          <span className={`px-2.5 py-1 rounded-sm text-xs font-mono font-medium ${camp.badgeColor} border border-white/30`}>
            Jednodenní
          </span>
        </div>
        ...
        <a
          ...
          className={`group inline-flex items-center justify-center px-6 py-3.5 ${camp.buttonBg} text-white font-bold rounded-md transition-all duration-300 hover:shadow-xl ${camp.buttonShadow} text-base`}
        >
          Přihlásit dítě
          ...
          <form onSubmit={handleEmailSubmit}>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-md text-sm text-ink placeholder-ink/40 bg-white focus:outline-none focus:ring-2 focus:ring-cta-500 font-mono min-w-0"
                required
              />
              <button
                type="submit"
                disabled={isSubmitting || !gdprConsent || !email.trim()}
                className="px-4 py-2 bg-white/20 hover:bg-white/30 text-white text-sm font-semibold rounded-md transition-colors disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
              >
...
export default function KvetenPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-950 via-primary-950 to-gray-950">
        ...
        {CAMPS.map((camp) => {
          ...
          return (
            <a
              key={camp.id}
              href={`#card-${camp.id}`}
              className={`flex items-center gap-2 px-3 py-2 rounded-md bg-white/10 border border-white/10 text-white/80 text-xs font-medium transition-colors hover:bg-white/20`}
            >
      ...
      >
        <div className="bg-white/5 backdrop-blur rounded-md p-6 border border-white/10">
          <p className="text-white/70 text-sm mb-4">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <a
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-md transition-colors text-sm"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-trust-600/30 hover:bg-trust-600/50 text-trust-300 rounded-md transition-colors text-sm"
            >
