...
export function Footer() {
  ...
  return (
    <footer className="bg-night border-t border-white/10 text-slate-300 relative overflow-hidden">
      ...
      <div className="section-container py-16 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div className="px-3 py-1.5 bg-night-800 rounded-lg text-xs font-medium text-slate-300">
              ...
              {location.isDefault && (
                <a
                  ...
                  className="px-3 py-1.5 bg-night-800 rounded-lg hover:bg-night-700 transition-colors flex items-center"
                >
                  <Image
            ...
            <a
              href={otherCity.href}
              className="mt-6 inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-accent-400 transition-colors"
            >
          ...
          {/* Quick Links */}
          <div>
            <p className="data-label mb-5 text-accent-400">02 / NAVIGACE</p>
            <ul className="space-y-3">
              {navLinks.map((link) => (
                <li key={link.name}>
                  <Link
                    href={link.href}
                    className="text-slate-400 hover:text-accent-400 transition-colors inline-flex items-center gap-2 group"
                  >
                    <span className="w-1.5 h-1.5 rounded-full bg-slate-600 group-hover:bg-accent-400 transition-colors" />
                    {link.name}
                  </Link>
                </li>
              ))}
          ...
          {/* Legal */}
          <div>
            <p className="data-label mb-5 text-accent-400">03 / PRÁVNÍ INFORMACE</p>
            <ul className="space-y-3">
              {legalLinks.map((link) => (
                <li key={link.name}>
                  <Link
          ...
          {/* Contact */}
          <div>
            <p className="data-label mb-5 text-accent-400">04 / KONTAKT</p>
              ...
              {location.venues.map((venue) => (
                <li key={venue.name} className="flex items-start gap-3">
                  ...
                  <span className="text-slate-400 text-sm">
                    <span className="text-white font-medium">{venue.name}</span><br />
        ...
        <div className="mt-24 pt-12 border-t border-white/10">
          <p
            className="font-display text-6xl md:text-8xl font-bold text-white/5 select-none text-center"
            aria-hidden="true"
          >
            WEEKS
          </p>
        ...
        {!location.isDefault && (
          <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row md:items-start md:justify-between gap-6">
            <div className="flex items-start gap-3 max-w-2xl">
              <FileText className="w-5 h-5 text-accent-400 shrink-0 mt-0.5" />
              <div className="text-sm text-slate-400 leading-relaxed">
                <p className="text-white font-medium mb-1">Provozovatel</p>
                <p>{location.organizer.fullName}</p>
                <p className="text-xs text-slate-500 mt-1">
                  Úplné kontaktní a fakturační údaje najdete v{' '}
                  <Link href={buildPath(location, 'podminky')} className="underline hover:text-slate-300">obchodních podmínkách</Link>.
                </p>
              </div>
            </div>
            <div className="shrink-0">
              <p className="text-xs text-slate-500 mb-2 md:text-right">Platby zajišťuje</p>
              <a
