  ...
  if (error || !registration) {
    return (
      <div className="max-w-lg mx-auto text-center py-20">
        ...
        <h1 className="heading-2 mb-2 text-white">Registrace nenalezena</h1>
        <p className="text-slate-300 mb-6">{error || 'Neplatný odkaz na registraci.'}</p>
    ...
    >
      ...
      <div className="text-center mb-8">
        {isPaid ? (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', duration: 0.5 }}>
            <CheckCircle2 className="w-20 h-20 text-trust-400 mx-auto mb-4" />
          </motion.div>
        ) : isPending ? (
          <Clock className="w-20 h-20 text-cta-400 mx-auto mb-4" />
        ) : (
          <XCircle className="w-20 h-20 text-red-400 mx-auto mb-4" />
        )}

        <h1 className="heading-2 mb-2 text-white">
      ...
      <div className="card-glow p-6 space-y-4">
        <div className="flex justify-between items-center pb-4 border-b border-white/10">
          <span className="text-sm text-slate-400">Stav</span>
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${
            isPaid ? 'bg-trust-500/20 text-trust-400' : isPending ? 'bg-cta-500/20 text-cta-400' : 'bg-red-500/20 text-red-400'
          }`}>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-slate-400">Dítě</p>
            <p className="font-medium text-white">{registration.child_name}</p>
          </div>
          <div>
            <p className="text-slate-400">Program</p>
            <p className="font-medium text-white">{registration.program}</p>
          </div>
          <div>
            <p className="text-slate-400">Termín</p>
            <p className="font-medium text-white">{new Date(registration.term_start).toLocaleDateString('cs-CZ', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
          </div>
          <div>
            <p className="text-slate-400">Částka</p>
            <p className="font-mono font-medium text-accent-400">{registration.payment_amount?.toLocaleString('cs-CZ')} Kč</p>
          </div>
        </div>
        <div className="pt-4 border-t border-white/10 text-sm">
          <p className="text-slate-400">Zákonný zástupce</p>
          <p className="font-medium text-white">{registration.parent_name}</p>
          <p className="text-slate-400">{registration.parent_email}</p>
        </div>
      </div>
