...
export function RegistrationForm() {
  ...
  function FieldError({ name }: { name: string }) {
    ...
    return <p role="alert" className="text-sm text-red-400 mt-1">{errors[0]}</p>
  }

  const inputClass = (name: string) =>
    `w-full px-4 py-3 rounded-lg border bg-night text-white placeholder:text-slate-500 ${fieldErrors[name] ? 'border-red-400/60 focus:border-red-400 focus:ring-1 focus:ring-red-400/50' : 'border-white/15 focus:border-accent-400 focus:ring-1 focus:ring-accent-400'} transition-colors`

  if (!program || !term) {
    return (
      <div className="max-w-lg mx-auto text-center py-20">
        <h1 className="heading-2 mb-4 text-white">Registrace</h1>
        <p className="text-slate-300 mb-6">Neplatný odkaz na registraci. Vyberte si tábor a termín.</p>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="text-center mb-8">
        <h1 className="heading-2 mb-2 text-white">Registrace na tábor</h1>
      ...
      <div className="flex items-center justify-between mb-10 max-w-sm mx-auto">
        {STEPS.map((s, i) => (
          <div key={s.id} className="flex items-center">
            ...
            {i < STEPS.length - 1 && (
              <div className={`w-6 sm:w-10 h-0.5 mx-1 ${step > s.id ? 'bg-primary-600' : 'bg-white/10'}`} />
            )}
          </div>
        ))}
      </div>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="mb-4 flex justify-center gap-2">
          <button
            ...
            className="px-4 py-2 text-xs font-mono bg-night-800 text-accent-400 rounded-lg border border-white/15 hover:border-accent-400/60 transition-colors"
          >
          ...
          <button
            type="button"
            onClick={() => setStep(1)}
            className="px-4 py-2 text-xs font-mono bg-night-800 text-cta-400 rounded-lg border border-white/15 hover:border-cta-400/60 transition-colors"
          >
            ↺ Krok 1
          </button>
        </div>
      ...
      <div className="card-glow p-6 sm:p-8">
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <p className="data-label mb-4">01 / ZÁKONNÝ ZÁSTUPCE</p>
              <h2 className="heading-2 text-white mb-6">Vaše údaje</h2>
              <div className="space-y-4">
                <div>
                  <label htmlFor="parent_name" className="block text-sm font-medium text-slate-300 mb-1">Jméno a příjmení *</label>
                ...
                <div>
                  <label htmlFor="parent_email" className="block text-sm font-medium text-slate-300 mb-1">E-mail *</label>
                ...
                <div>
                  <label htmlFor="parent_phone" className="block text-sm font-medium text-slate-300 mb-1">Telefon *</label>
                ...
                <div>
                  <label htmlFor="parent_address" className="block text-sm font-medium text-slate-300 mb-1">Fakturační adresa *</label>
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <p className="data-label mb-4">02 / DÍTĚ</p>
              <h2 className="heading-2 text-white mb-6">Údaje o dítěti</h2>
              <div className="space-y-4">
                <div>
                  <label htmlFor="child_name" className="block text-sm font-medium text-slate-300 mb-1">Jméno a příjmení *</label>
                ...
                <div>
                  <label htmlFor="child_birthdate" className="block text-sm font-medium text-slate-300 mb-1">Datum narození *</label>
                ...
                <div>
                  <label htmlFor="child_insurance" className="block text-sm font-medium text-slate-300 mb-1">Zdravotní pojišťovna *</label>
                ...
                <div>
                  <label htmlFor="child_health" className="block text-sm font-medium text-slate-300 mb-1">Zdravotní omezení / alergie</label>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Nepovinné. Uvádějte jen údaje nutné pro bezpečnost — použijeme je výhradně k zajištění bezpečné účasti dítěte na táboře.</p>
                </div>
                <div>
                  <label htmlFor="child_exp" className="block text-sm font-medium text-slate-300 mb-1">Zkušenosti s technologiemi</label>
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <p className="data-label mb-4">03 / VYZVEDNUTÍ</p>
              <h2 className="heading-2 text-white mb-6">Způsob vyzvednutí</h2>
              <div className="space-y-5">
                <div>
                  <p className="text-sm font-medium text-slate-300 mb-3">Jak bude dítě vyzvedáváno po skončení tábora? *</p>
                  <div className="space-y-3">
                    <label className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-colors ${pickup.pickup_method === 'solo' ? 'border-accent-400 bg-accent-400/10' : 'border-white/10 hover:border-white/15'}`}>
                      <input
                        type="radio"
                        name="pickup_method"
                        value="solo"
                      ...
                      />
                      <div>
                        <p className="font-medium text-white">Dítě odejde samo</p>
                        <p className="text-sm text-slate-400 mt-0.5">Dítě odejde po skončení programu samostatně</p>
                      </div>
                    </label>

                    <label className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-colors ${pickup.pickup_method === 'named_persons' ? 'border-accent-400 bg-accent-400/10' : 'border-white/10 hover:border-white/15'}`}>
                      <input
                        type="radio"
                        name="pickup_method"
                        value="named_persons"
                      ...
                      />
                      <div>
                        <p className="font-medium text-white">Vyzvedne jmenovaná osoba</p>
                        <p className="text-sm text-slate-400 mt-0.5">Dítě vyzvedne zákonný zástupce nebo níže uvedená osoba</p>
                      </div>
                    </label>
                  </div>
                ...
                {pickup.pickup_method === 'solo' && (
                  <motion.div key="pickup-time" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
                    <label htmlFor="pickup_time" className="block text-sm font-medium text-slate-300 mb-1">V kolik hodin smí dítě odejít? *</label>
                    <input
                      id="pickup_time"
                      type="time"
                      ...
                      max="17:00"
                    />
                    <p className="text-xs text-slate-500 mt-1">Standardně dítě odchází po skončení programu. Čas vyberte, jen pokud má odejít dříve.</p>
                ...
                {pickup.pickup_method === 'named_persons' && (
                  <motion.div key="pickup-persons" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
                    <label htmlFor="pickup_persons" className="block text-sm font-medium text-slate-300 mb-1">Kdo smí dítě vyzvednout? (nepovinné)</label>
                    <textarea
                      id="pickup_persons"
                    ...
                    />
                    <p className="text-xs text-slate-500 mt-1">Zákonný zástupce smí dítě vyzvednout vždy. Vyplňte, jen pokud bude dítě vyzvedávat někdo jiný — jméno a telefon každé osoby na samostatný řádek.</p>
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <p className="data-label mb-4">04 / SOUHLASY</p>
              <h2 className="heading-2 text-white mb-6">Povinné a volitelné souhlasy</h2>
              <div className="space-y-5">
                <div className="flex items-start gap-3">
                  <input type="checkbox" id="vop" checked={consents.vop_consent as boolean} onChange={e => setConsents({...consents, vop_consent: e.target.checked as unknown as true})} className="mt-1 w-5 h-5 shrink-0 rounded border-white/15 text-accent-500 bg-night accent-accent-400 focus:ring-accent-400" />
                  <label htmlFor="vop" className="text-sm text-slate-300">
                    Souhlasím s{' '}
                    <Link href={vopUrl} target="_blank" className="text-accent-400 underline hover:text-accent-300">Všeobecnými obchodními podmínkami</Link>
                    {' '}*
                  </label>
                </div>
                ...
                <div className="flex items-start gap-3">
                  <input type="checkbox" id="gdpr" checked={consents.gdpr_consent as boolean} onChange={e => setConsents({...consents, gdpr_consent: e.target.checked as unknown as true})} className="mt-1 w-5 h-5 shrink-0 rounded border-white/15 text-accent-500 bg-night accent-accent-400 focus:ring-accent-400" />
                  <label htmlFor="gdpr" className="text-sm text-slate-300">
                    Souhlasím se{' '}
                    <Link href={gdprUrl} target="_blank" className="text-accent-400 underline hover:text-accent-300">zpracováním osobních údajů</Link>
                ...
                <div className="flex items-start gap-3">
                  <input type="checkbox" id="photo" checked={consents.photo_consent} onChange={e => setConsents({...consents, photo_consent: e.target.checked})} className="mt-1 w-5 h-5 shrink-0 rounded border-white/15 text-accent-500 bg-night accent-accent-400 focus:ring-accent-400" />
                ...
                <div className="flex items-start gap-3">
                  <input type="checkbox" id="marketing" checked={consents.marketing_consent} onChange={e => setConsents({...consents, marketing_consent: e.target.checked})} className="mt-1 w-5 h-5 shrink-0 rounded border-white/15 text-accent-500 bg-night accent-accent-400 focus:ring-accent-400" />
          ...
          {step === 5 && (
            <motion.div key="step5" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <p className="data-label mb-4">05 / SHRNUTÍ</p>
              <h2 className="heading-2 text-white mb-6">Přehled registrace</h2>
              <div className="space-y-6">
                <div className="bg-night-800 border border-accent-400/20 rounded-xl p-4">
                  <h3 className="font-medium text-accent-300 mb-2">Tábor</h3>
                  <p className="text-white">{program.name}</p>
                  <p className="text-sm text-slate-400">{location.name} · {new Date(term.startDate).toLocaleDateString('cs-CZ', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                  <p className="font-mono text-lg font-bold text-accent-400 mt-2">{program.price.toLocaleString('cs-CZ')} Kč</p>
                </div>

                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <h3 className="font-medium text-white mb-2">Zákonný zástupce</h3>
                  ...
                  <div>
                    <h3 className="font-medium text-white mb-2">Dítě</h3>
                    <div className="text-sm text-slate-400 space-y-1">
                ...
                <div>
                  <h3 className="font-medium text-white mb-2">Vyzvednutí</h3>
                  <div className="text-sm text-slate-400 space-y-1">
                    {pickup.pickup_method === 'solo' && (
                ...
                <div className="pt-2 border-t border-white/10">
                  <label htmlFor="customer_note" className="block text-sm font-medium text-slate-300 mb-1">Poznámka k objednávce (nepovinné)</label>
                  <textarea
                    id="customer_note"
                    value={customerNote}
                    ...
                    maxLength={1000}
                    rows={3}
                    className="w-full px-4 py-3 rounded-lg border bg-night text-white placeholder:text-slate-500 border-white/15 focus:border-accent-400 focus:ring-1 focus:ring-accent-400 transition-colors"
                ...
                <p className="text-xs text-slate-500 pt-2 border-t border-white/10">
                  ...
                  <Link href={vopUrl} target="_blank" className="text-accent-400 underline hover:text-accent-300">VOP</Link>{' '}
                  a{' '}
                  <Link href={gdprUrl} target="_blank" className="text-accent-400 underline hover:text-accent-300">zpracováním osobních údajů</Link>.
        ...
        {/* Error */}
        {error && (
          <div role="alert" aria-live="assertive" className="mt-4 p-3 bg-red-500/10 border border-red-400/60 rounded-lg">
        ...
        <div className="flex justify-between mt-8 pt-6 border-t border-white/10">
          {step > 1 ? (
            <button onClick={prevStep} className="flex items-center gap-2 px-4 py-2 rounded-lg text-slate-400 hover:text-slate-300 transition-colors focus:outline-none focus:ring-2 focus:ring-accent-400 focus:ring-offset-2 focus:ring-offset-night">
          ...
          {step < 5 ? (
            ...
            <button onClick={handleSubmit} disabled={isSubmitting} className="btn-primary disabled:opacity-50 disabled:cursor-not-allowed">
              {isSubmitting ? (
                <>
