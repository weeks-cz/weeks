...
function getConfig(): FakturoidConfig | null {
  ...
  const userAgent = process.env.FAKTUROID_USER_AGENT || 'Weeks (admin@weeks.cz)'
...
export function buildSubjectPayload(p: InvoiceParams) {
  return {
    name: p.parentName,
    email: p.parentEmail,
    // We only collect a single free-text address; put it in `street` so it
    // renders on the document. country defaults to CZ.
    street: p.parentAddress,
    country: 'CZ',
    // custom_id lets us find the subject again in the Fakturoid UI by registration.
...
export function buildInvoicePayload(subjectId: number, p: InvoiceParams) {
  return {
    ...
    lines: [
      {
        name: `${p.programName} — ${p.termLabel}`,
        quantity: '1',
        unit_name: 'ks',
...
/**
 * Create a subject + paid invoice for a settled registration. Returns the
